...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
      <nav
        className={`w-full transition-all duration-300 backdrop-blur-2xl border-b shadow-md ${isPastHero
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          >
            <div className={`w-8 sm:w-9 h-8 sm:h-9 rounded-full overflow-hidden shrink-0 border shadow-md transition-colors ${isPastHero ? "border-slate-300 dark:border-white/25" : "border-white/25"
              }`}>
              <img
                src={logoImage}
          ...
          <div className="hidden lg:flex items-center gap-5 xl:gap-7">
            {navLinks.map((link) => (
              <a
                ...
                className={`text-xs xl:text-sm font-medium transition-colors cursor-pointer ${isPastHero
          ...
          <div className="hidden md:flex items-center gap-3.5">
            <div className={`h-5 w-[1px] hidden lg:block transition-colors ${isPastHero ? "bg-slate-300 dark:bg-white/20" : "bg-white/20"
              }`} />
            <Button
          ...
          <button
            className={`lg:hidden p-2 rounded-full transition-colors focus:outline-none ${isPastHero
          ...
          >
            {isOpen ? <X className="w-5 h-5 text-[#8B0000]" /> : <Menu className="w-5 h-5" />}
          </button>
        </div>

        {/* Mobile Navigation Dropdown */}
        {isOpen && (
          <div className={`lg:hidden py-4 px-4 sm:px-6 border-t backdrop-blur-2xl shadow-2xl animate-fade-up ${isPastHero
            ...
            <div className="flex flex-col gap-2">
              {navLinks.map((link) => (
                <a
                  key={link.id}
                  href={`#${link.id}`}
                  className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all cursor-pointer ${isPastHero
                      ? "text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-[#8B0000]"
